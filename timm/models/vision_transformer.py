...
class Block(nn.Module):
    ...
    def norm2ckpt(self, x: torch.Tensor) -> torch.Tensor:
        return checkpoint(self.norm2, x)

    def mlpckpt(self, x: torch.Tensor) -> torch.Tensor:
        return checkpoint(self.mlp, x)

    def ls2ckpt(self, x: torch.Tensor) -> torch.Tensor:
        return  checkpoint(self.ls2, x)

    def drop2ckpt(self, x: torch.Tensor) -> torch.Tensor:
        return  checkpoint(self.drop_path2, x)

    def block2(self, x: torch.Tensor) -> torch.Tensor:
        return x + self.drop2ckpt(self.ls2ckpt(self.mlpckpt(self.norm2ckpt(x))))
    
    def norm1ckpt(self, x: torch.Tensor) -> torch.Tensor:
        return checkpoint(self.norm1, x)

    def ls1ckpt(self, x: torch.Tensor) -> torch.Tensor:
        return checkpoint(self.ls1, x)

    def drop1ckpt(self, x: torch.Tensor) -> torch.Tensor:
        return checkpoint(self.drop_path1, x)
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        #   attn_mask=attn_mask)))
        xsave = x
        x = checkpoint(self.norm1, x)
        x = self.attn(x)
        x = checkpoint(self.ls1, x)
        x = checkpoint(self.drop_path1, x)
        #x = x + xsave
        x = torch.utils.checkpoint.checkpoint(self.add, checkpoint(self.drop_path1, x), xsave, use_reentrant=False,)

        
        '''xsave=x
        x = checkpoint(self.norm2, x)
        x = checkpoint(self.mlp, x)
        x = checkpoint(self.ls2, x)
        x = checkpoint(self.drop_path2, x) + xsave'''
